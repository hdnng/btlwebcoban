...
namespace WebApplication1.aspx
{
    ...
    {
        protected HtmlGenericControl Supplement; // Đổi tên biến để tránh mâu thuẫn
        ...
        private void LoadProducts()
        {
            ...
            if (products != null)
            {
                // Sửa lại để lọc sản phẩm theo danh mục Supplements
                List<Product> supplementProducts = products.Where(p => p.Category == "Supplements").ToList();

                foreach (var product in supplementProducts)
                {
                    string productHtml = $@"
                <div class='product-item'>
                    ...
                    <div class='product-item-buttons'>
                        <form method='post' action='Supplements.aspx'>
                    ...
                    Supplement.InnerHtml += productHtml;
                }
            }
            else
            {
                // Đảm bảo rằng biến ProductContainer không bị null
                if (Supplement == null)
                {
                    Supplement = new HtmlGenericControl("div") { ID = "Supplement" };
                }
            }
        }
